...
const generateTypes = (languageType: LanguageType, options: { errOnUnusedParam: boolean, includePlurals: boolean }) => (
  ...
  + 'export interface Language {'}${indent(generateTypeFragment(languageType, options))}\n}\n`
);

const generateTypeFragment = (languageType: LanguageType, options: { errOnUnusedParam: boolean, includePlurals: boolean }) => {
  let code = '';
  Object.entries(languageType).forEach(([key, languageTypeKey]) => {
    if (languageTypeKey.type === 'object') {
      code += `\n${key}: {${indent(generateTypeFragment(languageTypeKey.values, options))}\n},`;
